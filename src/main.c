...
void print_usage() {
    fprintf(stderr, "Usage: box_merge -i input_file -o output_file -u ucd_database\n");
}
...
int main(int argc, char* argv[]) {
    ...
    char_tree_t* tree = generate_ucd_data(ucd_database);
    merge_boxses(input_box_file, output_box_file, tree);

    /* while(i != NULL) { */
    /*     //printf("%d : %c\n", i->val.code,i->val.join_class); */
    /*     i = i->next; */
    /* } */
    /* Open Box File and process boxes*/
    //merge_boxes(argv[2],argv[3]);
    exit(EXIT_SUCCESS);
}
